...
export default function ChatApp() {
  ...
  const handleSend = async (text) => {
    ...
    try {
      ...
      if (!threadId) {
        ...
        const saveRes = await fetch(`http://localhost:5000/api/messages/threads/${threadId}/messages`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${stored.token}` },
      ...
      const replyRes = await fetch(
        `http://localhost:5000/api/messages/threads/${threadId}/ai-reply`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${stored.token}` },
          signal: controller.signal,
  ...
  const loadThread = async (threadId) => {
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/messages/threads/${threadId}`, {
